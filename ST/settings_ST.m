...
T11   = transl([0.5 0.15 0]);
Tf 	  = transl([0.5, 0.05, 0]);
...
ainit = timeseries(deltaXe_des',t(1:length(deltaXe_des)));
T_e_init = cardatol(tr2rpy(Hd.data(:,:,1)),1,2,3);      %xyz

xe_des2 = downsample(xe_des(:,2:end), ceil(dt_pilco/dt));
a_init{1} = [t_pilco(1:H), diff(xe_des2(:,2:end))];
...
multTargets(1,:) = [80 80];
multTargets(2,:) = [100 100];
multTargets(3,:) = [120 120];
varTargets = [160, 160, 0, 0];         % initial target variance
targets = [75, 75, 0, 0];          % targets for initializing hyperparameters

policy.fcn = @(policy,m,s)my_mixedConCat(@congp,@my_mixedGSat,policy,m,s);  % linear saturating controller
policy.maxU  = [150/2 150/2, 0.05  0.05];
policy.minU  = [10    10,   -0.05 -0.05];
...
opt.length = 35;                        % (+): max. number of line searches
