function [t_train, t_learn, t_pred] = timePILCOT(nc, poli, nii)
%% Inits
warning('on','all');
rng(0,'twister');
...
ref_select = dyno(1:end-1) - 2*n;                    %#ok<*NASGU> % indices of reference corresponding to dyno    [xe dxe F] (leave as is)
...
policy.p.inputs  = gaussian(zeros(1,length(poli)), diag(ones(1,length(poli))*0.1), nc)';                % policy pseudo inputs   [ N  x  d ]
