...
%     end
    ...
    if i==1
        ...
        ml(3) = plot(1,0.3,'b','visible','off','MarkerSize',10,'LineWidth', 1.5);       % record
        ml(4) = plot(1,0.3,'g','visible','off','MarkerSize',10,'LineWidth', 1);         % test
        ml(5) = plot(1,0.3,'y','visible','off','MarkerSize',10,'LineWidth', 1);         % full model  
        if compareToFullModel && ~isempty(Mfull{j})
            legend(ml,'Reference','Planning Model','Data Trial','Test Trials','Full Model');
        else
