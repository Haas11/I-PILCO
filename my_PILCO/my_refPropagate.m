...
function [Mnext, Snext] = propagate(m, s, plant, dynmodel, policy, varargin)
%% Code
global diffChecks diffTol gpCheck REF_DIFF REF_PRIOR FIRST
...
if isempty(prevRefVel) || t==1  || FIRST
    prevRefVel = zeros(1,length(policy.refIdx))';
...
if REF_PRIOR
    if ~isfield(policy,'refIdx') || isempty(policy.refIdx);
        Mnext = Mnext + REF_DIFF(t,:)';                                     % recenter around pre-computed reference

    elseif ~isempty(policy.refIdx)
        
	% Mean:
        ma = M(D1+1:D2);                                                    % computed actions @t
        deltaRef_pos= ma(policy.refIdx,1);                                 % action = change in postion reference = ref_t+1 - ref_t        
        curRefPos 	= prevRefPos + deltaRef_pos;
        curRefVel 	= deltaRef_pos/plant.dt; % reference velocity = change in pos ref / sampling time        
		curRef 		= [curRefPos; curRefVel; zeros(mod(length(plant.dyno),2),1)];
        deltaRef_vel= curRefVel - prevRefVel;        
		curDeltaRef = [deltaRef_pos; deltaRef_vel; zeros(mod(length(plant.dyno),2),1)];    % (zeros for force indices)
		
		refPrior = curRef; refPrior(difi,1) = curDeltaRef(difi,1);
        
        Mnext = Mnext + refPrior;                                           % Recentered next state
