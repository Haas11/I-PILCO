...
if isfield(policy,'remap') && policy.remap
    scale_factor = (policy.maxU'.*(3/2));
    M = M./scale_factor;             % map unsaturated control output to the interval: [-pi/2, pi/2]
end
...
if nargout > 3
    ...
    if isfield(policy,'remap') && policy.remap  % adjust derivatives for rescaling
        dMdm = dMdm./repmat(scale_factor,1,D);
        dMdp = dMdp./repmat(scale_factor,1,E*D+E);
    end
end
