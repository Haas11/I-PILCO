...
function [f, df] = my_hypCurb(lh, covfunc, x, y, curb)
%% Code
if nargin < 5
    curb.snr = 500; 
    curb.ls = 100; 
...
if size(lh,1) == 3*D+2
    li = 1:2*D; 
    sfi = 2*D+1:3*D+1;      % 1D and DD terms
elseif size(lh,1) == 2*D+1
    li = 1:D; 
    sfi = D+1:2*D;          % Just 1D terms
elseif size(lh,1) == D+2
    li = 1:D; 
    sfi = D+1;              % Just DD terms
else
...
% lightspeed + Double toolbox sometimes give complex values, this prevents
% this:
% f = f + sum(realpow(((ll - log(curb.std'))./log(curb.ls)),p));          % length-scales
% df(li) = df(li) + p*realpow((ll - log(curb.std')),(p-1))/log(curb.ls)^p;
% 
% f = f + sum(realpow(((lsf - lsn)/log(curb.snr)),p));                    % signal to noise ratio
% df(sfi) = df(sfi) + p*realpow((lsf - lsn),(p-1))/log(curb.snr)^p;
% df(end) = df(end) - p*sum( realpow((lsf - lsn),(p-1)) /log(curb.snr)^p);
