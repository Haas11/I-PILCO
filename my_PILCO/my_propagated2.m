...
function [Mnext, Snext, Mcon, Scon, Ccon, dMdm, dSdm, dMds, dSds, dMdp, dSdp] = ...
  my_propagated2(m, s, plant, dynmodel, policy, t)
%% Code
global gradTypes diffChecks REF_DIFF REF_PRIOR

if nargout <= 5                                  % just predict, no derivatives
  [Mnext, Snext, Mcon, Scon, Ccon] = my_propagate2(m, s, plant, dynmodel, policy, t);
  return
end
...
for n=1:Nf                               % potentially multiple dynamics models
  ...
  j = [j k];                                   %#ok<AGROW> % update 'previous' state vector
end
