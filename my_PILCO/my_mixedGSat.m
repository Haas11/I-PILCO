...
% 
% edit: translates part of the action space to non-negative values.
%
%    function [M, S, C, dMdm, dSdm, dCdm, dMdv, dSdv, dCdv] = my_gSat(m, v, i, e)
%
% *Input arguments:*
%
...
%   e     (optional) scale vector; default: 1                        [1   x  I ]
% 	policy 
%
% *Output arguments:*
%
...
if isfield(policy, 'impIdx') && ~isempty(policy.impIdx)				% partial translation
	translIdx = policy.impIdx;
else 
	translIdx = [];
end
e_mixed = [e(1,translIdx) + policy.minU(translIdx), zeros(1,length(e)-length(translIdx))];

%if isfield(policy, 'translIdx') && ~isempty(policy.translIdx)				% partial translation
%	translIdx = policy.translIdx;
%	e_mixed = [e(1,policy.translIdx), zeros(1,length(e)-length(policy.translIdx))];
%end
...
M = P*M2;                                                            % mean
M = M + e_mixed';   % translate mapping vertically upwards to enforce positivity on non-negative part of action space
