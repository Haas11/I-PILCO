...
function [Mnext, Snext, Mcon, Scon, Ccon] = my_propagate2(m, s, plant, dynmodel, policy, t)
%% Code
global diffChecks diffTol gpCheck REF_PRIOR REF_DIFF
...
% M(k) = dynmodel.inputs(t,end-1:end);
% S(k,k) = zeros(2,2);
% S(j,k) = zeros(size(q));
% S(k,j) = zeros(size(q'));
...
% [mTest, sTest, ~]  = policy.fcn(policy, mm(i), zeros(length(i)));
