...
% handles.config.Mode.Mode = -1;      % REVERTS BACK TO POSITION CONTROL!!!! indicate that control mode will remain the same (time saving)
...
%     q0 = [0; 0.25; 0; -1.43; 0; 1.46; 0];   % [0.45 0 0.4]         
    q0 = [0; 0.21; 0; -1.75; 0; 1.19; 0];     % [0.45 0 0.3]
%     q0 = mu0(1:7);
...
while ~response.Success
    ...
    handles.config.Mode.CartesianStiffness.Stiffness.X = 500;    % TODO: change for different action dimensionality!
    handles.config.Mode.CartesianStiffness.Stiffness.Y = 500;
    handles.config.Mode.CartesianStiffness.Stiffness.Z = 500;
    response = call(handles.client, handles.config, 'Timeout', 1);
    
    if response.Success
        latestPose   = receive(handles.subCartesianPose);
        latestPose.Pose.Position.X = latestPose.Pose.Position.X - 0.2;
%         latestPose.Pose.Position.Y = latestPose.Pose.Position.Y + 0.05;
