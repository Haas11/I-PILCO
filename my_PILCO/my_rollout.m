...
%     x0trial = [mu0(plant.dyno([1 2])), 0]';
    x0trial = mu0(2*robot.n+1:2*robot.n+6)';
    clear ref dref
    if initTrial
        % do nothing
    else
        x0trial = x0trial + plant.startStateInterval.*(2*rand(6,1)-1);  % perturb by uniform random amount [-S0Trial, +S0Trial]
    end
    Hd0 = rt2tr(rpy2r(x0trial(4:6)'),x0trial(1:3));
...
%         q0 = robot.ikcon(Hd0,q0est);        % w/ joint limits
        q0 = robot.ikine(Hd0,q0est,[1 1 0 0 0 1],'pinv');
    end
    Hd0 = robot.fkine(q0);
    x0trial = transl(Hd0);
