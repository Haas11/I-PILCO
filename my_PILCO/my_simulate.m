...
function next = my_simulate(x0, f, plant)
%% Code
...
(strcmp(con,'foh') && tau+delay<=dt) || (strcmp(con,'lag') && delay==0)
    ...
    x0s = x0(1:end-2*nU); 
    U   = [reshape( x0(end-2*nU+1:end), [nU 2]) f]; 
    id  = 2;
end
ctrlfcn = str2func(con); 
u0 = cell(1,nU);        % create control function
for j = 1:nU 
...
if id==0,     
    next =  x1;                         % return augmented state
elseif id==1, 
    next = [x1; udt];
else
    next = [x1; f; udt];
